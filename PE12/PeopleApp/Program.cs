...
{
    // Class: PeopleApp
    // Author: David Schuh (additions made by Ajay Ramnarine for PE12)
    // Purpose: include PeopleLib and CourseLib and create a catalogue (?) of students and teacher course info
    // Restrictions: None
    class PeopleApp
    {
        // Method: Main
        // Purpose: Create the list of courses as well as people objects
        //          Give the user an interface to either add, edit, delete, list, live, or quit the application
        // Restrictions: None
        static void Main(string[] args)
        {
            // creates a new list of 100 courses between IGME-200 and IGME-299
                    ...
                    case "list":
                        ...
                        foreach (KeyValuePair<string, Person> thisEntry in people.sortedList)
                        {
                            ...
                            if (thisPerson.GetType() == typeof(Student))
                            {
                                ...
                                // writes out the list of student courses along witht he schedules of the courses
                                foreach (string cCode in student.courseCodes) 
                                {
                                    Console.Write(cCode + " - ");
                                    if (courses[cCode] == null) 
                                    {
                                        Console.WriteLine("Error! Could not find course.");
                                    }
                                    else
                                    {
                                        Console.Write(courses[cCode].description + " ");
                                        for(int j = 0; j < courses[cCode].schedule.daysOfWeek.Count; j++) 
                                        {
                                            Console.Write(courses[cCode].schedule.daysOfWeek[j] + " ");
                                        }
                                        Console.WriteLine("{0:hh:mmtt} - {1:hh:mmtt}", courses[cCode].schedule.startTime, courses[cCode].schedule.endTime);
        ...
        // Function: EditPerson
        // Purpose: Calls to edit the person if the user choooses to edit a person that has been added to the database
        // Restrictions: None
        public static void EditPerson(ref Person thisPerson)
        {
            ...
            if (thisPerson.GetType() == typeof(Student))
            {
                ...
                // adds the course codes that the user enters if they are a student
                do
                {
                    ...
                    if (cCode.Length > 0)
                    {
                        if (!thisStudent.courseCodes.Contains(cCode))
                        {
                            thisStudent.courseCodes.Add(cCode);
                        }
                    }
                    else 
                    {
